...
def load_data():
    ...
    if db_url:
        ...
        # Наша таблица для истории диалогов
        cur.execute("""
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    data = query.data
    user_id = query.from_user.id  # Узнаем ID юзера, нажимающего кнопки
    ...
    if data == "idea":
        ...
        prompt = f"Придумай одну конкретную идею для поста в рубрике «{rubric}». Кратко: tema + почему зайдёт аудитории предпринимателей 40-50+."
        
        # ИСПРАВЛЕНО: вызываем функцию с памятью
        idea_text = ask_gemini_with_memory(user_id, prompt)
        ...
        # ИСПРАВЛЕНО: вызываем функцию с памятью
        post_text = ask_gemini_with_memory(user_id, prompt)
        ...
        if "idea" in data:
            ...
            # ИСПРАВЛЕНО: вызываем функцию с памятью
            text = ask_gemini_with_memory(user_id, prompt)
